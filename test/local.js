...
describe('Local', function () {
  var bsLocal;
  beforeEach(function () {
    bsLocal = new testgrid.Local();
  });

  it('should have pid when running', function (done) {
    this.timeout(600000);
    bsLocal.start({ 'key': process.env.TESTGRID_ACCESS_KEY }, function(){
  ...
  it.skip('should throw error on running multiple binary', function (done) {
    this.timeout(60000);
    bsLocal.start({ 'key': process.env.TESTGRID_ACCESS_KEY }, function(error){
      bsLocal_2 = new testgrid.Local();
      var tempLogPath = path.join(process.cwd(), 'log2.log');

      bsLocal_2.start({ 'key': process.env.TESTGRID_ACCESS_KEY, 'logfile': tempLogPath }, function(error){
        expect(error.toString().trim()).to.equal('LocalError: Either another testgrid local client is running on your machine or some server is listening on port 45690');
  ...
  it('should enable verbose', function (done) {
    bsLocal.start({ 'key': process.env.TESTGRID_ACCESS_KEY, onlyCommand: true, 'verbose': true }, function(){
  ...
  it('should enable verbose with log level', function (done) {
    bsLocal.start({ 'key': process.env.TESTGRID_ACCESS_KEY, onlyCommand: true, 'verbose': 2 }, function(){
      expect(bsLocal.getBinaryArgs().indexOf('--verbose')).to.not.equal(-1);
  ...
  it('should set folder testing', function (done) {
    bsLocal.start({ 'key': process.env.TESTGRID_ACCESS_KEY, onlyCommand: true, 'f': '/var/html' }, function(){
  ...
  it('should set folder testing with folder option', function (done) {
    bsLocal.start({ 'key': process.env.TESTGRID_ACCESS_KEY, onlyCommand: true, 'folder': '/var/html' }, function(){
      expect(bsLocal.getBinaryArgs().indexOf('-f')).to.not.equal(-1);
      expect(bsLocal.getBinaryArgs().indexOf('/var/html')).to.not.equal(-1);
      done();
    });
  ...
  it('should enable force', function (done) {
    bsLocal.start({ 'key': process.env.TESTGRID_ACCESS_KEY, onlyCommand: true, 'force': true }, function(){
  ...
  it('should enable only', function (done) {
    bsLocal.start({ 'key': process.env.TESTGRID_ACCESS_KEY, onlyCommand: true, 'only': true }, function(){
  ...
  it('should enable onlyAutomate', function (done) {
    bsLocal.start({ 'key': process.env.TESTGRID_ACCESS_KEY, onlyCommand: true, 'onlyAutomate': true }, function(){
  ...
  it('should enable forcelocal', function (done) {
    bsLocal.start({ 'key': process.env.TESTGRID_ACCESS_KEY, onlyCommand: true, 'forcelocal': true }, function(){
  ...
  it('should enable custom boolean args', function (done) {
    bsLocal.start({ 'key': process.env.TESTGRID_ACCESS_KEY, onlyCommand: true, 'boolArg1': true, 'boolArg2': true }, function(){
  ...
  it('should enable custom keyval args', function (done) {
    bsLocal.start({ 'key': process.env.TESTGRID_ACCESS_KEY, onlyCommand: true, 'customKey1': 'custom value1', 'customKey2': 'custom value2' }, function(){
  ...
  it('should enable forceproxy', function (done) {
    bsLocal.start({ 'key': process.env.TESTGRID_ACCESS_KEY, onlyCommand: true, 'forceproxy': true }, function(){
  ...
  it('should set localIdentifier', function (done) {
    bsLocal.start({ 'key': process.env.TESTGRID_ACCESS_KEY, onlyCommand: true, 'localIdentifier': 'abcdef' }, function(){
  ...
  it('should set parallelRuns', function (done) {
    bsLocal.start({ 'key': process.env.TESTGRID_ACCESS_KEY, onlyCommand: true, 'parallelRuns': '10' }, function(){
  ...
  it('should set hosts', function (done) {
    bsLocal.start({ 'key': process.env.TESTGRID_ACCESS_KEY, onlyCommand: true, 'only': 'localhost,8000,0'}, function(){
      expect(bsLocal.getBinaryArgs().indexOf('--only')).to.not.equal(-1);
...
describe('Start sync', () => {
  ...
  it('should have pid when running', function () {
    this.timeout(60000);
    bsLocal.startSync({ 'key': process.env.TESTGRID_ACCESS_KEY});
    expect(bsLocal.tunnel.pid).to.not.equal(0);
  });
  ...
  it.skip('should throw error on running multiple binary', function () {
    ...
    bsLocal_2 = new testgrid.Local();
    var tempLogPath = path.join(process.cwd(), 'log2.log');
    const error = bsLocal_2.startSync({ 'key': process.env.TESTGRID_ACCESS_KEY, 'logfile': tempLogPath });
    expect(error.toString().trim()).to.equal('LocalError: Either another testgrid local client is running on your machine or some server is listening on port 45690');
    fs.unlinkSync(tempLogPath);
  });
  ...
  describe('Download Path', function() {
    ...
    it('should return download path of darwin binary', function() {
      ...
      osNames.forEach(function(os) {
        ...
        expect(localBinary.getDownloadPath()).to.equal('https://www.testgrid.com/local-testing/downloads/binaries/TestGridLocal-darwin-x64');
      });
    });
    ...
    it('should return download path of exe binary', function() {
      ...
      osNames.forEach(function(os) {
        sandBox.stub(localBinary, 'hostOS', os);
        expect(localBinary.getDownloadPath()).to.equal('https://www.testgrid.com/local-testing/downloads/binaries/TestGridLocal.exe');
      });
    });
    ...
    it('should return download path of linux 64 arch binary', function() {
      ...
      expect(localBinary.getDownloadPath()).to.equal('https://www.testgrid.com/local-testing/downloads/binaries/TestGridLocal-linux-x64');
    });
    ...
    it('should return download path of linux 32 arch binary', function() {
      ...
      localBinary.isAlpine = sandBox.stub(localBinary, 'isAlpine').returns(false);
      expect(localBinary.getDownloadPath()).to.equal('https://www.testgrid.com/local-testing/downloads/binaries/TestGridLocal-linux-ia32');
    });
    ...
    it('should return download path of alpine linux binary', function() {
      ...
      sandBox.stub(localBinary, 'is64bits', true);
      expect(localBinary.getDownloadPath()).to.equal('https://www.testgrid.com/local-testing/downloads/binaries/TestGridLocal-alpine');
    });
