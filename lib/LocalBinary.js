...
function LocalBinary(){
  ...
  this.getDownloadPath = function () {
    if(this.hostOS.match(/darwin|mac os/i)){
      return 'https://www.testgrid.com/local-testing/downloads/binaries/TestGridLocal-darwin-x64';
    } else if(this.hostOS.match(/mswin|msys|mingw|cygwin|bccwin|wince|emc|win32/i)) {
      this.windows = true;
      return 'https://www.testgrid.com/local-testing/downloads/binaries/TestGridLocal.exe';
    } else {
      if(this.is64bits) {
        if(this.isAlpine())
          return 'https://www.testgrid.com/local-testing/downloads/binaries/TestGridLocal-alpine';
        else
          return 'https://www.testgrid.com/local-testing/downloads/binaries/TestGridLocal-linux-x64';
      } else {
        return 'https://www.testgrid.com/local-testing/downloads/binaries/TestGridLocal-linux-ia32';
      }
    }
  };
  ...
  this.downloadSync = function(conf, destParentDir, retries) {
    ...
    var destBinaryName = (this.windows) ? 'TestGridLocal.exe' : 'TestGridLocal';
  ...
  this.getAvailableDirs = function(){
    ...
    throw new LocalError('Error trying to download TestGrid Local binary');
  };
  ...
  this.orderedPaths = [
    path.join(this.homedir(), '.testgrid'),
    process.cwd(),
    os.tmpdir()
  ];
}
